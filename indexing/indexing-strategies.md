...
The `EXPLAIN` shows `type` as `range` for conditions like inequality `BETWEEN` etc., but on the other hand it also show the same `range` for `IN()` caluse, even though it is multiple equality condition.

In the exaple above, tha last condition can be raplaced with `IN (18, 19, 20, 21, 22, 23, 24, 25)`, it will allow the index to be used, but it worth noting that lots of in statementes, can behave badly when the number of combinations )generated by `IN` statements approaches thousands)
