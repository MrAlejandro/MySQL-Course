...
* Filesort on small result sets in usually fine, but for large sets it performs badly.
* Special index can be added fro low selectivity columns specially for improuving soring performance

```sql 
ALTER TABLE profiles ADD KEY (sex, rating);
SELECT <cols> FROM profiles WHERE sex = 'M' ORDER BY rating LIMIT 10;
```

* Pagination may make the situation much worse `SELECT <cols> FROM profiles WHERE sex = 'M' ORDER BY rating LIMIT 10 LIMIT 100000, 10`
* Pagination case can be handled by denormalizin, precumputing and caching
* Deep pagination might be limit on the application side
* Defferd join might help
